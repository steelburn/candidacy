...
JWT_SECRET=...                  # JWT authentication secret (shared across all services)
JWT_TTL=60                      # JWT access token time-to-live in minutes (default: 60)
JWT_REFRESH_TTL=20160          # JWT refresh token time-to-live in minutes (default: 2 weeks = 20160)
```

### JWT Configuration

The Candidacy platform uses **JWT (JSON Web Tokens)** via the `tymon/jwt-auth` package for authentication.
...
| `JWT_ALGO` | HS256 | Algorithm for token signing |
| `JWT_BLACKLIST_ENABLED` | true | Enable token blacklist (for logout) |
| `JWT_BLACKLIST_GRACE_PERIOD` | 0 | Grace period for tokens in blacklist |
...
- **JWT Secret Management**: Ensure `JWT_SECRET` is at least 32 characters. Generate using `make generate-secrets` or `php artisan jwt:secret`
- **JWT Token TTL**: Adjust `JWT_TTL` based on security requirements (shorter = more secure, longer = better UX)
